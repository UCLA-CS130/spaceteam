...
# thread_output=$(curl -I http://localhost:2020/hold & (curl -I http://localhost:2020/echo) | (sleep 10 & pkill webserver))

# # Checks that the diff produces no output, since the files should be the same.
# if [ $thread_output -eq "Waiting on HoldingHandler" ];
# 	then
...
# Will return 1 if all tests passed, will return 0 if one fails
exit $result
